...
|---|---|---|---|---|---|---|---|
| `accounting/audit-logs` | `/accounting/audit-logs` | Create `AccountingAuditLogsView`, controller, service for `/api/accounting/audit-logs` | `GET /api/accounting/audit-logs` | Audit Log Table with JSON diff sheet (old vs new) | High | `AccountingService` | **Completed (Post-Sync)** |
| `accounting/health` | `/accounting/health` | Create `AccountingHealthView`, controller, service for `/api/accounting/health-check` | `GET /api/accounting/health-check`, `POST /api/accounting/repost-missing` | Diagnostic cards, severity badges, one-click repost/fix buttons | High | `AccountingService` | **Completed (Post-Sync)** |
| `accounting/reconciliation` | `/accounting/reconciliation` | Create 4-tabbed `AccountingReconciliationView` (Ledger, Cash & Bank, Parties, GST) | `GET/POST /api/accounting/reconciliation/*` | 4 Tabs: Ledgers, Cash & Bank, Parties, GST with fix actions | High | `AccountingService` | **Completed (Post-Sync)** |
| `accounting/bank-statement-import` | `/accounting/bank-statement-import` | Create `BankStatementImportView` with PDF upload, transaction mapping, stepper wizard, and one-click voucher posting | `POST /api/accounting/bank-statement/import`, `POST /api/accounting/bank-statement/:id/post-entries` | Statement upload card, auto-mapped status badges, stepper wizard, one-click post button | Medium | `AccountingService` | **Completed (Post-Sync)** |
